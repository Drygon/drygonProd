export const coursematerials = [
  {
    item: 1,
    subject: "Presentation",
    text: [
      {
        item: 1,
        description: "Power Point Presentation (23 Slides)"
      }
    ]
  },
  {
    item: 2,
    subject: "Technical",
    text: [
      {
        item: 1,
        description: "Summary Description of the Design Basis Memorandum (DBM) Phase"
      },
      {
        item: 2,
        description: "Summary Description of the Engineering Design Specification (EDS) Phase"
      },
      {
        item: 3,
        description: "Front End Loading (FEL) According to IPA"
      },
      {
        item: 4,
        description: "Value Improving Practices (VIP’s)",
        link: "https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FValue%20Improving%20Practices%20-%20FINAL.pdf?alt=media&token=e5cdcb24-4877-4f7b-98b4-f09d369a8a13",
      },
      {
        item: 5,
        description: "Functional Analysis System Technique (FAST) Diagrams"
      }
    ]
  },
  {
    item: 3,
    subject: "Insights",
    text: [
      {
        item: 1,
        description: "Guiding Principles of Project Execution",
        link:"https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FGuiding%20Principles%20of%20Project%20Execution.pdf?alt=media&token=63f8d9ee-361a-4815-a66b-f1f719ebb06e"
      },
      {
        item: 2,
        description: "Potential Pitfalls for Project Managers"
      },
      {
        item: 3,
        description: "Ground Rules for Meetings"
      },
      {
        item: 4,
        description: "Summary of 7 Habits of Highly Effective People"
      },
      {
        item: 5,
        description: "Eight Simple Anger Management Tips"
      },
      {
        item: 6,
        description: "Rules for Retirement",
        link:"https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FRules%20For%20Retirement.pdf?alt=media&token=7c1607f3-4a9b-44be-a5f3-9e9670566b2f"
      }
    ]
  },
  {
    item: 4,
    subject: "Quality & Safety",
    text: [
      {
        item: 1,
        description: "Quality Management Works – Presented at 1996 PMI Symposium",
        link:"https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FPMI%20QMW%20-%20Article.pdf?alt=media&token=08f35bca-5d64-40ae-8839-7697fb8b07cc",
      },
      {
        item: 2,
        description: "A Quality Approach to Projects"
      },
      {
        item: 3,
        description: "Synopsis of Deming’s 14 Points on Total Quality Management"
      },
      {
        item: 4,
        description: "Safety in Engineering Designs",
        link: "https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FSafety%20in%20Engineering%20Designs%20-%20Final.pdf?alt=media&token=17813132-0752-425d-a6fd-6ef86ea6b0e2",
      },
      {
        item: 5,
        description: "Safety and Speeding"
      },
      {
        item: 6,
        description: "What Is Your Safety Score?"
      }
    ]
  },
  {
    item: 5,
    subject: "Philosophical",
    text: [
      {
        item: 1,
        description: "Words From the Dalai Lama",
        link:"https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FHandout%20-%20Dalai%20Lama.pdf?alt=media&token=040aef68-94c8-4067-a996-339e6f1be36e"
      },
      {
        item: 2,
        description: "The Law of Least Effort"
      },
      {
        item: 3,
        description: "The Balance In Your Life"
      },
      {
        item: 4,
        description: "Life’s Lessons"
      },
      {
        item: 5,
        description: "The American Dream",
        link:"https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FThe%20American%20Dream.pdf?alt=media&token=c14bd47f-fe64-4c2c-95d5-3690694de660"
      },
      {
        item: 6,
        description: "Happiness"
      },
      {
        item: 7,
        description: "Soft and Strong "
      },
      {
        item: 8,
        description: "Owning or Being Owned"
      }
    ]
  },
  {
    item: 6,
    subject: "Whimsical",
    text: [
      {
        item: 1,
        description: "Problem Solving Flowsheet",
        link:"https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FProblem%20Solving%20Flowsheet.pdf?alt=media&token=2e3c562d-8de7-4a63-ad69-41b1264b521d"
      },
      {
        item: 2,
        description: "Laws of Project Management"
      },
      {
        item: 3,
        description: "Alternate Dictionary Definitions"
      },
      {
        item: 4,
        description: "People Who Count!"
      }
    ]
  },
  {
    item: 7,
    subject: "The Charts",
    text: [
      {
        item: 1,
        description: "Cartoon on Work Processes",
        link: "https://firebasestorage.googleapis.com/v0/b/drygonprod.appspot.com/o/Training%2FEureka%20Cartoon.pdf?alt=media&token=2c95c3e4-3582-4e69-9298-de09bf91d88e",
      },
      {
        item: 2,
        description: "Self- Test with Answers"
      },
      {
        item: 3,
        description: "DCI-100-A: Discipline Interface Chart During Design Basis Memorandum (DBM) Phase"
      },
      {
        item: 4,
        description: "DCI-100-B: Discipline Interface Chart During EDS & Detailed Engineering Phases"
      }
    ]
  }
];
